...
density <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) +
  ...
  labs(
    title = "Density of Ozone by year",
    x = "Ozone AQI value",
    y = "ozone density"
  )
print(density)
...
boxplot <- ggplot(EPAair, aes(x = as.factor(Year),
                              y = Ozone))+
  geom_boxplot() +
...
cp <- 
  ...
  na.omit()
cpCorr <- cor(cp)
corrplot(cpCorr, method = "ellipse")
corrplot.mixed(cpCorr, upper = "ellipse")
...
ozoneAIC <- lm(data = EPAair, PM2.5 ~ Ozone + Year + Month + mean_Lng + mean_Lat)
